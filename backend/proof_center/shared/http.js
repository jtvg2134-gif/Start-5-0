...
export function getRoutePath(rawPathname = "") {
  ...
  if (pathname.startsWith("/api/start5-proof-center")) {
    return pathname.slice("/api/start5-proof-center".length) || "/";
  }
...
export function sendRouteError(response, error) {
  ...
  console.error("Start5 Proof Center API error:", error);
  sendJson(response, 500, { error: "Erro interno do backend Proof Center." });
}
